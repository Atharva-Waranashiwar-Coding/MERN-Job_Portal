...
import { Table, TableHead, TableBody, TableRow, TableCell, Button, Box, Typography, CircularProgress, Pagination } from '@mui/material';
...
const UsersPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/user/getAll?page=${page}&limit=${usersPerPage}`);
  ...
  return (
    <Box className="container" display="flex" flexDirection="column"  justifyContent="top !important" height="100vh">
